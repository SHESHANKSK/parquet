    ...
    <div class="flex flex-col gap-4 p-6 bg-white rounded-2xl border border-zinc-200 shadow-sm">
      ...
      @if (parquetService.error()) {
        <div class="p-3 bg-red-50 border border-red-100 rounded-lg flex items-start gap-2">
          <i class="text-red-500 scale-75 mt-0.5">error_outline</i>
      ...
      <div class="flex flex-wrap gap-2">
        ...
        <button (click)="setExample(1)"
                class="text-[10px] px-2 py-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-md transition-colors">
          Select All
        </button>
        <button (click)="setExample(2)"
                class="text-[10px] px-2 py-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-md transition-colors">
          Filter & Sort
        </button>
        <button (click)="setExample(3)"
                class="text-[10px] px-2 py-1 bg-zinc-100 hover:bg-zinc-200 text-zinc-600 rounded-md transition-colors">
          Aggregation
        </button>
      </div>
